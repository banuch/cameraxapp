...
class MeterDetector(private val context: Context) {
    ...
    init {
        ...
        // Load default model (first one found or fallback to MAIN_MODEL.tflite)
    ...
    /**
     * Get the current confidence threshold for detection filtering
     */
     ...
     * @param visualize Whether to draw detection boxes on the result image
     * @return Pair of detection results and processed bitmap
     *
     * Input processing:
     ...
     */
    fun detectMeterReading(bitmap: Bitmap, visualize: Boolean = false): Pair<List<DetectionResult>, Bitmap> {
        ...
        // Draw detections only if visualization is enabled
        val resultBitmap = if (visualize) {
            drawDetections(inputBitmap, detections)
        } else {
            inputBitmap
        }
    ...
    private fun loadModelFile(): MappedByteBuffer {
        return loadModelFile(currentModel?.fileName ?: "MAIN_MODEL.tflite")
    }
